...
// type FeedingSchedulerService struct {
// 	db *gorm.DB
// }

// func NewFeedingSchedulerService(db *gorm.DB) *FeedingSchedulerService {
// 	return &FeedingSchedulerService{db: db}
// }
...
// func (s *FeedingSchedulerService) CreateFeedingLogsForItems(items []ItemToFeed, dateTime time.Time, note string, createdBy uint) error {
...
//	}
type FeedingSchedulerService struct {
	feedingScheduleRepo FeedingRepositoryScheduler
}

func NewFeedingSchedulerService(feedingScheduleRepo FeedingRepositoryScheduler) *FeedingSchedulerService {
	return &FeedingSchedulerService{feedingScheduleRepo: feedingScheduleRepo}
}
...
// CreateFeedingLogsForItems คือ Logic หลักที่รับ "หลายรายการ" และทำงานใน Transaction เดียว
func (s *FeedingSchedulerService) CreateFeedingLogsForItems(items []ItemToFeed, dateTime time.Time, note string, createdBy uint) error {
	return s.feedingScheduleRepo.DeductStockAndLogFeeding(items, dateTime, note, createdBy)
}

func (s *FeedingSchedulerService) GetSchedulesDue(timeStr string) ([]models.FeedingSchedule, error) {
	return s.feedingScheduleRepo.GetSchedulesByTime(timeStr)
}
